...
- 飞书通道：CardKit 流式状态卡片（channel config `stream_card`，默认关闭）——轮次过程（思考中 / 工具调用 / 进度提示 / 最终回复）投影到单张可原地刷新的卡片，轮次结束自动定稿；CardKit 不可用或回复超长时自动回退普通消息发送。

### 修复

- 飞书通道：修复保存设置后旧 WebSocket 连接未关闭的问题；检测连接时仅校验凭据，避免干扰正在收消息的通道。
- 飞书通道：修复"处理中"表情回应在轮次结束后不撤回的问题（删除钩子兼容批量合并消息，按会话清扫）。
- 飞书通道：进度提示间隔默认从 120 秒收紧为 60 秒。
